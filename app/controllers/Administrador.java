...
public class Administrador extends BaseController {
	...
	public static void editLayout(String message) {
		String banner = "";
		
		render(banner);
	}
	
	public static void sendArchive(@Valid(message="form.application.upload.error") File arquivo) {
		if(validation.hasErrors()) {
			params.flash();
			flash.keep();
		...
		String nameFile = arquivo.getName();
		File path = new File(Messages.get("application.path.upload.images", ""));

		if(new File(path, nameFile).exists()) {
			Logger.info("###### Arquivo do Banner removido? %s ######", new File(path, nameFile).delete());
		}
		
		Logger.info("###### Arquivo [%s] enviado? %s ######", nameFile, arquivo.renameTo( new File(path, nameFile) ));
		
		editLayout("Imagem enviada com sucesso!");
	}
